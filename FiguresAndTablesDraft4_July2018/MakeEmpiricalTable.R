library(ape)
library(BMhyb)
load("/Users/bomeara/Documents/MyDocuments/GitClones/BMhyb_paper/CichlidDraft4_July2018/CichlidResults_AllCombined.RData")
cichlid.phy <- read.tree("/Users/bomeara/Documents/MyDocuments/GitClones/BMhyb_paper/CichlidDraft4_July2018/cichlid.phy")
results.cichlid <- all.results
load("/Users/bomeara/Documents/MyDocuments/GitClones/BMhyb_paper/NicotianaDraft4_July2018/NicotianaResults_AllCombined.RData")
nicotiana.phy <- read.tree("/Users/bomeara/Documents/MyDocuments/GitClones/BMhyb_paper/NicotianaDraft4_July2018/nicotiana.phy")
results.nicotiana<- all.results
results.nicotiana$error <- c(rep("Zero", 4), rep("Inferred",4))
for (i in sequence(ncol(results.nicotiana))) {
  results.nicotiana[,i] <- unlist(results.nicotiana[,i])
}
results.nicotiana$deltaAICc <- results.nicotiana$AICc - min(results.nicotiana$AICc)
results.nicotiana$AkaikeWeight <- AkaikeWeight(results.nicotiana$deltaAICc)
...
write.csv(output.table, file="/Users/bomeara/Documents/MyDocuments/GitClones/BMhyb_paper/FiguresAndTablesDraft4_July2018/EmpiricalResults.csv")
