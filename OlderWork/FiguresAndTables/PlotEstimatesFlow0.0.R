...
for (figure.index in sequence(2)) {
	...
	if(figure.index==1) {
		...
		pdf.name <- "/Users/bomeara/Dropbox/CollabJhwuengOMeara/PCM_Hybridization/Resubmission/NewFiguresTables/SimulationSummaries/FigureSimulationResultsFlow0.0.pdf"
		logs.to.use <- logs.to.use.main.fig
		summaries.by.estimator <- summaries.by.estimator.main
	} else {
		params.to.plot <- params.to.plot.supp.fig
		pdf.name <- "/Users/bomeara/Dropbox/CollabJhwuengOMeara/PCM_Hybridization/Resubmission/NewFiguresTables/SimulationSummaries/SuppFigureSimulationResultsFlow0.0.pdf"
